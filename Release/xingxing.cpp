...
void music()
{
	PlaySound(TEXT("sound\\yue.WAV"),0,SND_ASYNC|SND_LOOP);  //循环播放
}
...
void Playing(int x,int y)
{
	...
	int k,p,q,c=0,d,e,g=0,n,h=0,o=0,t,w;
		...
		for(i=0;i<9;i++)
		{
			for(j=0;j<10;j++)
			{
				if(x>=(10+j*40)&&y>=(110+i*40)&&x<=(10+(j+1)*40)&&y<=(110+(i+1)*40))//点击方块
				{
					p=i;
					q=j;
					setlinestyle(PS_SOLID,5);//设置填充模式和颜色
					...
					if(i-1>=0)//从第2行开始
					{
						if(cell[i-1][j]==k||cell[i-1][j]==k+5||cell[i-1][j]==k+10||cell[i-1][j]==k-10||cell[i-1][j]==k-5)//遍历颜色相同的方块
						{
		...
		for(j=0;j<10;j++)
		{
			...
			for(i=0;i<9;i++)//从第j列第1行开始往下循环遍历至第9行
			{
				if(cell[i][j]==110)
				{
					c++;
			...
			for(d=0;d<(c-g);d++)//开始进行循环 控制的循环的次数（c-g为掉落的行数）
			{
				for(i=e;i>=d;i--)
				{
					cell[i][j]=cell[i-1][j];//向下掉落
				}
				cell[d][j]=110;//掉落后的第d行第j列的方块为白色
			}
			c=0;
			g=0;
			select_color(cell);
		...
		for(j=0;j<10;j++)
		{
			...
			if(n==9)//如果为最后一列（第10列）则最后一列直接变为白色，不向左移
			{
				h=0;
			}
			for(d=0;d<h;d++)//开始进行循环 控制的循环的次数
			{
				for(i=0;i<9;i++)//循环到第9行
				{
					for(j=n-h+1;j<10;j++)//从第一个白块开始 到最后一个（n-h+1为向左移的列数）
					{
						cell[i][j]=cell[i][j+1];//向左移动
					}
					cell[i][9-d]=110;//左移后的第i行第9-d列的色块为白色
				}
			}
			h=0;
			select_color(cell);
